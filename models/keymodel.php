...
class KeyModel extends CI_Model {
    ...
    function getKey($keyid) {
        $this->db->select("k.KeysID, k.Title AS Name, k.UID, k.Description, k.Rank, k.TaxonomicScope, k.GeographicScope, k.Notes,
    ...
    public function getItemInfo($itemid) {
        $this->db->select('ItemsID, Name AS ItemName');
        $this->db->from('items');
        $this->db->where('itemsID', $itemid);
        $query = $this->db->get();
        if ($query->num_rows())
    ...
    public function getChanges($keyid) {
        $this->db->select("CONCAT_WS(' ', u.FirstName, u.LastName) AS full_name,TimestampModified AS timestamp_modified, c.`comment`", FALSE);
